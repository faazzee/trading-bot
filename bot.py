...
from scheduler import (
    check_price_alerts, check_news_alerts, check_opportunities,
    check_fundamental_and_sentiment_alerts, send_weekly_digest,
)
...
from sentiment_engine import compute_30day_sentiment, load_finbert
...
━━━ *Long-Term Analysis* ━━━
/longterm `SYMBOL` — 3-month+ composite score \\(fundamentals \\+ sentiment \\+ technicals\\)
/fundamental `SYMBOL` — Fundamental health scorecard
/sentiment `SYMBOL`  — 30-day FinBERT news sentiment
...
async def longterm_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text(
            "❌ Usage: /longterm SYMBOL\nExample: `/longterm AAPL`",
            parse_mode="Markdown",
        )
        return
    ...
    msg = await update.message.reply_text(
        f"🔬 Computing long-term score for *{symbol}*… (this may take ~30s while FinBERT analyses headlines)",
        parse_mode="Markdown",
    )

    result = await asyncio.to_thread(compute_long_term_score, symbol)
...
async def fundamental_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text(
            "❌ Usage: /fundamental SYMBOL\nExample: `/fundamental MSFT`",
            parse_mode="Markdown",
        )
        return

    symbol = context.args[0].upper()
    msg    = await update.message.reply_text(f"🔍 Fetching fundamentals for *{symbol}*…", parse_mode="Markdown")

    fundamentals = await asyncio.to_thread(get_fundamentals, symbol)
    if "error" in fundamentals:
        await msg.edit_text(f"❌ Could not fetch fundamentals for *{symbol}*.", parse_mode="Markdown")
        return

    scoring = await asyncio.to_thread(score_fundamentals, fundamentals)

    def _pct(v, is_ratio=True):
        if v is None:
            return "N/A"
    ...
    rec_val = fundamentals.get("recommendation_mean")
    ...
    text = (
        ...
        f"🎯 *Score: {scoring.get('total_score', 0):.0f}/100 — {scoring.get('label', 'N/A')}*\n\n"
        f"*── Growth ──*\n"
        f"• EPS Growth:       {_pct(fundamentals.get('eps_growth'))}\n"
        f"• Revenue Growth:   {_pct(fundamentals.get('revenue_growth'))}\n\n"
        f"*── Valuation ──*\n"
        f"• Forward P/E:      {_val(fundamentals.get('forward_pe'))}\n"
        f"• PEG Ratio:        {_val(fundamentals.get('peg_ratio'))}\n\n"
        f"*── Quality ──*\n"
        f"• Profit Margin:    {_pct(fundamentals.get('profit_margin'))}\n"
        f"• Return on Equity: {_pct(fundamentals.get('return_on_equity'))}\n"
        f"• Debt/Equity:      {_val(fundamentals.get('debt_to_equity'))}\n\n"
        f"*── Analyst View ──*\n"
        f"• Target Upside:    {upside_str}\n"
...
async def sentiment_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    symbol = context.args[0].upper()
    msg    = await update.message.reply_text(
        f"🧠 Analysing news sentiment for *{symbol}*… (FinBERT is scoring headlines)",
        parse_mode="Markdown",
    )

    result = await asyncio.to_thread(compute_30day_sentiment, symbol)
    ...
    label_emoji = {"Positive": "🟢", "Negative": "🔴", "Neutral": "🟡"}.get(label, "🟡")
    trend_emoji = {"Improving": "📈", "Declining": "📉", "Stable": "➡️"}.get(trend, "➡️")

    lines = [
        f"🧠 *News Sentiment — {symbol}*\n",
        f"*Score: {score:.0f}/100 — {label_emoji} {label}*",
        f"Trend: {trend_emoji} {trend} (over last 30 days, {count} headlines analysed)\n",
        f"*── Top Headlines ──*",
    ]
    ...
    if headlines:
        for h in headlines[:5]:
            ...
            hemoji    = {"Positive": "🟢", "Negative": "🔴", "Neutral": "🟡"}.get(hlabel, "🟡")
            safe_title = title.replace("_", "\\_")
            lines.append(f"{hemoji} _{safe_title}_")
    else:
        lines.append("No headlines analysed yet.")
...
def main():
    ...
    scheduler.add_job(
        check_fundamental_and_sentiment_alerts, "interval",
        minutes=FUNDAMENTAL_SENTIMENT_CHECK_INTERVAL,
        args=[app],
        id="fundamental_sentiment_alerts",
        max_instances=1,
    )
    scheduler.add_job(
    ...
    async def post_init(application: Application) -> None:
        ...
        logger.info(
            ...
            f"fundamentals/sentiment every {FUNDAMENTAL_SENTIMENT_CHECK_INTERVAL}m, "
            f"weekly digest {WEEKLY_DIGEST_DAY} at {WEEKLY_DIGEST_HOUR}:00 UTC"
        )
